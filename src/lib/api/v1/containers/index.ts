...
export const containersApi = {
	...
	delete: (id: string, force = false) =>
		api.delete<void>(`/api/v1/containers/${id}?force=${force}`),
	...
	updateResources: (id: string, body: ResourceLimits) =>
		api.patch<{ message: string; restart_required: boolean }>(
			`/api/v1/containers/${id}/resources`,
			body
		),

	deployCompose: (req: DeployComposeRequest) =>
		api.post<DeployComposeResponse>(`/api/v1/containers/compose`, req),
