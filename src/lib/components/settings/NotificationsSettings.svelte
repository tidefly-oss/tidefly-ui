<script lang="ts">
    ...
    const testMutation = createMutation(() => ({
        ...
        onSuccess: (_: unknown, channel: string) =>
            toast.success(`Test sent via ${channel}`),
        onError: (_: unknown, channel: string) =>
            toast.error(`Test failed via ${channel}`),
    }));
...
<div class="space-y-4">
    <!-- Master toggle -->
    <div
        class="rounded-xl border bg-card px-5 py-4 flex items-center justify-between"
    >
        <div>
        ...
        <button
            ...
            class="relative w-9 h-5 rounded-full transition-colors shrink-0 {enabled
                ? 'bg-primary'
                : 'bg-muted border border-border'}"
        >
            <div
                class="absolute top-0.5 left-0.5 size-4 rounded-full bg-white shadow-sm transition-transform {enabled
                    ? 'translate-x-4'
                    : ''}"
            ></div>
        </button>
    ...
    <div class="rounded-xl border bg-card divide-y">
        <div
            class="px-5 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wide"
        >
            Channels
        </div>
        ...
        <div class="px-5 py-4 space-y-3">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                    <div
                        class="size-7 rounded-lg bg-purple-500/10 flex items-center justify-center"
                    >
                        <svg
        ...
        <div class="px-5 py-4 space-y-3">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                    <div
                        class="size-7 rounded-lg bg-indigo-500/10 flex items-center justify-center"
                    >
                        <svg
        ...
        <div class="px-5 py-4">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                    <div
                        class="size-7 rounded-lg bg-muted flex items-center justify-center"
                    >
                    ...
                    <div>
                        ...
                        <p class="text-xs text-muted-foreground">
                            Uses SMTP settings from the SMTP tab
                        </p>
                    </div>
                </div>
                {#if smtpConfigured}
        ...
        {#each triggers as t (t.key)}
            <div class="px-5 py-3.5 flex items-center justify-between gap-4">
                ...
                <button
                    ...
                    class="relative w-9 h-5 rounded-full transition-colors shrink-0 {triggerValue(
                        t.key,
                    )
                        ? 'bg-primary'
                        : 'bg-muted border border-border'}"
                >
                    <div
                        class="absolute top-0.5 left-0.5 size-4 rounded-full bg-white shadow-sm transition-transform {triggerValue(
                            t.key,
                        )
                            ? 'translate-x-4'
                            : ''}"
