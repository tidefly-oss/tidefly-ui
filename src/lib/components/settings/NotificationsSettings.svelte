...
$effect(() => {
	if (settings) {
		inAppEnabled = settings.notifications_enabled ?? false;
		externalEnabled = settings.external_notifications_enabled ?? false;
...
function buildPayload() {
	return {
		notifications_enabled: inAppEnabled,
		external_notifications_enabled: externalEnabled,
...
async function saveAndTest(channel: string) {
...
const triggers = [
	{
		...
		desc: "When a service or container is deployed",
	},
	{
		key: "container",
		label: "Container down",
		desc: "When a container stops unexpectedly",
	},
	{
		key: "webhook",
		label: "Webhook delivery failed",
		desc: "When a webhook job fails",
	},
] as const;
...
function triggerToggle(key: (typeof triggers)[number]["key"]) {
	...
	else if (key === "container") notifyOnContainerDown = !notifyOnContainerDown;
	else notifyOnWebhookFail = !notifyOnWebhookFail;
}
</script>

{#snippet toggle(checked: boolean, onchange: () => void, label: string)}
    <button
            aria-label={label}
            type="button"
            role="switch"
            ...
            onclick={onchange}
            class="relative w-9 h-5 rounded-full transition-colors shrink-0 {checked
			? 'bg-primary'
			: 'bg-muted border border-border'}"
    >
        <span
                class="absolute top-0.5 left-0.5 size-4 rounded-full bg-white shadow-sm transition-transform {checked
				? 'translate-x-4'
				: ''}"
        ></span>
    </button>
...
<div class="space-y-4">
    ...
    <div class="rounded-xl border bg-card px-5 py-4 flex items-center justify-between">
        <div>
            <h2 class="text-sm font-semibold">In-App Notifications</h2>
            <p class="text-xs text-muted-foreground mt-0.5">
                Bell icon and notification feed in the dashboard
            </p>
        </div>
        {@render toggle(inAppEnabled, () => (inAppEnabled = !inAppEnabled), "Toggle in-app notifications")}
    </div>

    <!-- External Alerts master toggle -->
    <div class="rounded-xl border bg-card px-5 py-4 flex items-center justify-between">
        <div>
            <h2 class="text-sm font-semibold">External Alerts</h2>
            <p class="text-xs text-muted-foreground mt-0.5">
                Send alerts via Slack, Discord, or email
            </p>
        </div>
        {@render toggle(externalEnabled, () => (externalEnabled = !externalEnabled), "Toggle external alerts")}
    </div>

    {#if externalEnabled}
        <!-- Channels -->
        <div class="rounded-xl border bg-card divide-y">
            <div class="px-5 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wide">
                Channels
            </div>
            ...
            <div class="px-5 py-4 space-y-3">
                <div class="flex items-center justify-between">
                    ...
                    {#if slackConfigured}
						<span
                                class="inline-flex items-center gap-1 text-xs text-emerald-500 bg-emerald-500/10 px-2 py-0.5 rounded-full"
                        >
							<CircleCheckBig class="size-3" /> Configured
						</span>
                    {:else}
						<span
                                class="inline-flex items-center gap-1 text-xs text-muted-foreground bg-muted px-2 py-0.5 rounded-full"
                        >
