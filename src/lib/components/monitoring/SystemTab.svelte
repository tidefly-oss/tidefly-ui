<script lang="ts">
    ...
    import { CpuIcon, HardDriveIcon, NetworkIcon, ActivityIcon, ClockIcon } from '@lucide/svelte';
    ...
    const cpu       = $derived(metricsQuery.data?.cpu_percent  ?? overviewQuery.data?.resources.cpu_percent      ?? null);
    const mem       = $derived(metricsQuery.data?.mem_percent  ?? overviewQuery.data?.resources.memory.percent   ?? null);
    const disk      = $derived(metricsQuery.data?.disk_percent ?? overviewQuery.data?.resources.disk.percent     ?? null);
    const memUsed   = $derived(metricsQuery.data?.mem_used_mb  ?? overviewQuery.data?.resources.memory.used_mb   ?? null);
    const memTotal  = $derived(metricsQuery.data?.mem_total_mb ?? overviewQuery.data?.resources.memory.total_mb  ?? null);
    const diskUsed  = $derived(metricsQuery.data?.disk_used_mb  ?? overviewQuery.data?.resources.disk.used_mb    ?? null);
    const diskTotal = $derived(metricsQuery.data?.disk_total_mb ?? overviewQuery.data?.resources.disk.total_mb   ?? null);
    ...
    const memConfig  = { memory: { label: 'Memory %', color: 'var(--chart-2)' } } satisfies Chart.ChartConfig;
    const diskConfig = { disk:   { label: 'Disk %',   color: 'var(--chart-3)' } } satisfies Chart.ChartConfig;

    function xFmt(v: Date) {
        return v.toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' });
    }
    function tooltipFmt(v: Date) {
        return v.toLocaleString('de-DE', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' });
    }
