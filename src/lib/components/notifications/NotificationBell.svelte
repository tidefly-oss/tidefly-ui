...
<div class="relative" bind:this={dropdownRef}>
  ...
  {#if open}
    <div class="absolute right-0 top-11 z-50 flex w-105 flex-col rounded-lg border border-border bg-background shadow-xl">
      ...
      <div class="max-h-120 overflow-y-auto">
        ...
        {:else}
          {#each unread as n (n.id)}
            <div class="group flex gap-3 border-b border-border/50 px-4 py-3 transition-colors hover:bg-accent/30">
              <div class="mt-1.5 shrink-0">
                <span class="block h-2 w-2 rounded-full {severityDot(n.severity)}"></span>
              </div>
              <button class="min-w-0 flex-1 cursor-pointer text-left" onclick={() => goToContainerLogs(n)}>
                <span class="mb-1 flex items-center gap-2">
                  <span class="rounded border px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide {severityClass(n.severity)}">
                    {n.severity}
                  </span>
                  <span class="truncate font-mono text-xs text-muted-foreground">{n.container_name}</span>
                  {#if n.occurrence_count > 1}
                    <span class="ml-auto shrink-0 text-xs font-medium text-muted-foreground">×{n.occurrence_count}</span>
                  {/if}
                </span>
                <span class="line-clamp-2 text-xs leading-relaxed text-foreground/80 group-hover:text-foreground transition-colors">
                  {n.message}
                </span>
                <span class="mt-1 block text-[10px] text-muted-foreground/60">{formatTime(n.updated_at)}</span>
              </button>
              <div class="flex shrink-0 flex-col gap-1 justify-center">
                <button
                        onclick={(e) => { e.stopPropagation(); notificationsStore.acknowledge(n.id); }}
      ...
      {#if unread.length > 0}
        <div class="border-t border-border px-4 py-2 text-center">
          <a href="/dashboard/containers" class="text-xs text-muted-foreground hover:text-foreground" onclick={() => (open = false)}>
            View all containers →
          </a>
        </div>
      {/if}
