<script lang="ts">
  ...
  import {
    ...
    CircleIcon, CircleX, Loader, RocketIcon, TerminalIcon,
  } from "@lucide/svelte";
...
<div class="max-w-2xl mx-auto space-y-6">
  ...
  {:else if step === 2}
    <div class="bg-card border rounded-xl p-6 space-y-4">
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <div class="space-y-1.5">
          ...
          <select id="df-restart" bind:value={restart} class="border-input bg-background flex h-9 w-full rounded-md border px-3 py-1 text-sm shadow-sm">
    ...
    <div class="flex justify-between">
      <Button variant="outline" onclick={() => (step = 1)}><ArrowLeftIcon class="size-4 mr-1.5" /> Back</Button>
      <Button onclick={() => (step = 3)} disabled={!step2Valid}>Continue <ArrowRightIcon class="size-4 ml-1.5" /></Button>
    </div>
  ...
  {:else if step === 3}
    ...
    <div class="flex justify-between">
      <Button variant="outline" onclick={() => (step = 2)}><ArrowLeftIcon class="size-4 mr-1.5" /> Back</Button>
      <Button onclick={handleBuild} disabled={!step3Valid}><RocketIcon class="size-4 mr-1.5" /> Build & Deploy</Button>
    </div>
  ...
  {:else if step === 4}
    <div class="bg-card border rounded-xl overflow-hidden">
      <div class="px-5 py-4 border-b flex items-center gap-3">
        ...
        {#if status === "building"}
          <span class="flex items-center gap-1.5 text-xs text-blue-500 ml-auto"><Loader class="size-3 animate-spin" /> Building…</span>
        {:else if status === "success"}
          <span class="flex items-center gap-1.5 text-xs text-green-500 ml-auto"><CircleCheckBig class="size-3" /> Done</span>
        {:else if status === "error"}
          <span class="flex items-center gap-1.5 text-xs text-destructive ml-auto"><CircleX class="size-3" /> Failed</span>
        {/if}
      </div>
    ...
    <div class="flex gap-3">
      {#if status === "error"}
        <Button variant="outline" onclick={() => (step = 3)} class="flex-1"><ArrowLeftIcon class="size-4 mr-1.5" /> Edit Dockerfile</Button>
        <Button onclick={handleBuild} class="flex-1"><RocketIcon class="size-4 mr-1.5" /> Retry</Button>
      {:else if status === "success"}
        <Button variant="outline" onclick={() => goto("/dashboard/containers")} class="flex-1">View Containers</Button>
        <Button onclick={() => { step = 1; projectId = ""; name = ""; tag = ""; port = ""; logs = []; status = "idle"; }} class="flex-1">Deploy Another</Button>
      {/if}
    </div>
  {/if}
</div>