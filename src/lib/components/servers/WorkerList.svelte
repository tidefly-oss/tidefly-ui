<script lang="ts">
    ...
    let { workers, isLoading, onAddServer, onWorkerRevoked } = $props();
...
<div class="bg-card border rounded-xl overflow-hidden">
    <div
        class="flex items-center justify-between px-5 py-3.5 border-b bg-muted/20"
    >
        <div class="flex items-center gap-2.5">
            ...
            {#if !isLoading}
                ...
                >
                    {workers.filter((w: WorkerNode) => w.status === "connected")
                        .length} online
                </span>
            {/if}
        </div>
        <Tooltip.Provider delayDuration={100}>
            <Tooltip.Root>
                <Tooltip.Trigger>
                    <div
                        class="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors cursor-default px-2 py-1 rounded-md hover:bg-muted"
                    >
                ...
                <Tooltip.Content
                    class="max-w-64 text-xs leading-relaxed"
                    side="bottom"
                    align="end"
                >
                    All Plane ↔ Worker communication is encrypted with mutual
                    TLS. Certificates are issued by the Tidefly internal CA and
                    auto-renewed 30 days before expiry. Revoking a worker
                    immediately invalidates its certificate.
                </Tooltip.Content>
            </Tooltip.Root>
        </Tooltip.Provider>
    </div>
    ...
    {#if !isLoading && workers.length > 0}
        <div
            class="grid grid-cols-[2.5fr_1fr_1fr_1fr_auto] gap-4 px-5 py-2 border-b"
        >
    ...
    <div class="divide-y">
        {#if isLoading}
            {#each Array(2) as _, i (i)}
                <div
                    class="grid grid-cols-[2.5fr_1fr_1fr_1fr_auto] gap-4 px-5 py-4 items-center animate-pulse"
                >
        ...
        {:else if workers.length === 0}
            <div
                class="px-5 py-16 flex flex-col items-center gap-3 text-center"
            >
                ...
                <div>
                    ...
                    <p class="text-xs text-muted-foreground mt-1">
                        Add a server to deploy containers across multiple
                        machines
                    </p>
                </div>
        ...
        {:else}
            {#each workers as worker (worker.id)}
                ...
                <div
                    class="grid grid-cols-[2.5fr_1fr_1fr_1fr_auto] gap-4 px-5 py-3.5 items-center group hover:bg-muted/20 transition-colors {isRevoked
                        ? 'opacity-50'
                        : ''}"
                >
                    <!-- Server info -->
                        ...
                        <div class="min-w-0">
                            <div class="flex items-center gap-2 flex-wrap">
                                    ...
                                    <Tooltip.Root>
                                        <Tooltip.Trigger class="text-left">
                                            <span class="font-medium text-sm"
                                                >{shortName(worker.name)}</span
                                            >
                                        </Tooltip.Trigger>
                                        {#if worker.name.length > 32}
                                            <Tooltip.Content
                                                class="font-mono text-xs"
                                                >{worker.name}</Tooltip.Content
                                            >
                                        {/if}
                                    </Tooltip.Root>
                                </Tooltip.Provider>
                                <span class="flex items-center gap-1 shrink-0">
                                    <span
                                        class="size-1.5 rounded-full {statusDot(
                                            worker.status,
                                        )}"
                                    ></span>
                                    <span
                                        class="text-xs {statusText(
                                            worker.status,
                                        )}">{statusLabel(worker.status)}</span
                                    >
                                </span>
                            </div>
                            <div
                                class="flex items-center gap-2 mt-0.5 text-xs text-muted-foreground flex-wrap"
                            >
                                {#if worker.last_seen_ip}
                                    <span class="font-mono"
                                        >{worker.last_seen_ip}</span
                                    >
                                {/if}
                                ...
                                {#if worker.runtime_type}
                                    <span class="capitalize"
                                        >{worker.runtime_type}</span
                                    >
                                {/if}
                                {#if worker.agent_version}
                                    <span class="font-mono"
                                        >v{worker.agent_version}</span
                                    >
                                {/if}
                            </div>
                        </div>
                    ...
                    <div class="flex items-center gap-1.5 text-xs">
                        ...
                        {#if isConnected}
                            <span
                                class="tabular-nums {metricColor(
                                    worker.cpu_percent,
                                )}">{fmtPct(worker.cpu_percent)}</span
                            >
                        {:else}
                    ...
                    <div class="flex items-center gap-1.5 text-xs">
                        ...
                        {#if isConnected}
                            <span
                                class="tabular-nums {metricColor(
                                    worker.mem_percent,
                                )}">{fmtPct(worker.mem_percent)}</span
                            >
                        {:else}
                            <span class="text-muted-foreground">—</span>
                        {/if}
                    ...
                    <span class="text-xs text-muted-foreground tabular-nums"
                        >{timeAgo(worker.last_seen_at)}</span
                    >
                                ...
                                <Tooltip.Root>
                                    <Tooltip.Trigger>
                                        <Button
                                            ...
                                            onclick={() =>
                                                goto(
                                                    `/dashboard/servers/${worker.id}/logs`,
                                                )}
                                        >
...
>
    <Dialog.Content class="max-w-sm">
        <Dialog.Header>
            ...
            <Dialog.Description>
                Revoke <strong>{revokeTarget?.name}</strong>? The agent will be
                disconnected and its mTLS certificate immediately invalidated.
        ...
        <Dialog.Footer>
            ...
            >
                {#if revokeMutation.isPending}<Loader
...
>
    <Dialog.Content class="max-w-sm">
        <Dialog.Header>
            ...
            <Dialog.Description>
                Permanently remove <strong>{deleteTarget?.name}</strong> from the
                list? This only deletes the record — the certificate is already revoked.
            </Dialog.Description>
        </Dialog.Header>
        <Dialog.Footer>
            ...
            >
                {#if deleteMutation.isPending}<Loader
                        class="size-4 mr-2 animate-spin"
                    />{/if}
                Delete
            </Button>
