...
const latestProgress = $derived(
	updateStore.progressMessages[updateStore.progressMessages.length - 1]?.message ?? ""
);
</script>
	...
	<Dialog.Content class="max-w-3xl flex flex-col max-h-[85vh]">
		<Dialog.Header class="shrink-0 pb-4">
			<div class="flex items-center gap-4">
				<div class="rounded-xl p-3 shrink-0 {updateStore.updateDone ? 'bg-green-500/10' : updateStore.updateError ? 'bg-destructive/10' : 'bg-primary/10'}">
			...
			<div class="shrink-0 grid gap-2 mb-4">
				{#each updateStore.components as component (component.name)}
					<div class="flex items-center gap-3 px-3 py-2.5 rounded-lg border border-border bg-muted/30">
						<span class="text-base shrink-0">{componentIcon[component.name] ?? "📦"}</span>
						<div class="flex-1 min-w-0">
		...
		{#if updateStore.updating}
			<div class="shrink-0 mb-4 rounded-lg border border-border bg-muted/20 p-4 space-y-3">
				<div class="flex items-center gap-2">
					<Loader class="size-4 animate-spin text-primary shrink-0" />
					<span class="text-sm font-medium">Updating — do not close this window</span>
				</div>
				{#if latestProgress}
					<p class="text-xs text-muted-foreground font-mono truncate">{latestProgress}</p>
				{/if}
				<div class="space-y-1 max-h-32 overflow-y-auto">
					{#each updateStore.progressMessages as msg (msg.ts)}
						<p class="text-xs text-muted-foreground font-mono">{msg.message}</p>
					{/each}
				</div>
			</div>
		{/if}
		...
		<!-- Success state -->
		{#if updateStore.updateDone}
			<div class="shrink-0 mb-4 rounded-lg border border-green-500/20 bg-green-500/5 p-4 flex items-center gap-3">
				<CheckCircle2Icon class="size-5 text-green-500 shrink-0" />
				<div>
					<p class="text-sm font-medium text-green-500">All components updated successfully</p>
					<p class="text-xs text-muted-foreground mt-0.5">The page will reload shortly.</p>
				</div>
			</div>
		{/if}
