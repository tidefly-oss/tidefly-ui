<script lang="ts">
    ...
    const guideSteps = $derived(
        providerSteps[provider] ?? providerSteps.github,
    );
            ...
            {#each steps as label, i (i)}
                ...
                <div class="flex items-center gap-0 flex-1 last:flex-none">
                    <div class="flex items-center gap-2 shrink-0">
                        <div
                            class="size-6 rounded-full flex items-center justify-center text-xs font-medium border-2 transition-all
                            {isDone
            ...
            <div class="space-y-4">
                <div class="space-y-1.5">
                    ...
                    <Input
                        id="wh-name"
                        bind:value={name}
                        placeholder="e.g. Deploy on push to main"
                    />
                </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div class="space-y-1.5">
                        ...
                        <Select.Root type="single" bind:value={triggerType}>
                            ...
                            <Select.Content>
                                <Select.Item value="redeploy"
                                    >Redeploy service</Select.Item
                                >
                                <Select.Item value="deploy"
                                    >Full deploy</Select.Item
                                >
                            </Select.Content>
                        </Select.Root>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <Select.Root type="single" bind:value={provider}>
                            <Select.Trigger class="w-full"
                                >{providerLabel}</Select.Trigger
                            >
                            <Select.Content>
                                <Select.Item value="github">GitHub</Select.Item>
                                <Select.Item value="gitlab">GitLab</Select.Item>
                                <Select.Item value="gitea"
                                    >Gitea / Forgejo</Select.Item
                                >
                                <Select.Item value="bitbucket"
                                    >Bitbucket</Select.Item
                                >
                                <Select.Item value="generic"
                                    >Generic</Select.Item
                                >
                            </Select.Content>
                        </Select.Root>
                ...
                <div class="space-y-1.5">
                    <Label for="wh-branch">
                        ...
                        <span
                            ...
                            >(optional — empty = all branches)</span
                        >
                    </Label>
                    <Input
                        id="wh-branch"
            ...
            <Dialog.Footer class="mt-5">
                ...
                <Button onclick={submit} disabled={saving || !step1Valid}>
                    {#if saving}
                        Creating…
                    {:else}
                        Create & Continue <ArrowRightIcon
                            class="size-4 ml-1.5"
                        />
                    {/if}
                </Button>
            ...
            <div class="space-y-4">
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="flex items-center gap-2">
                        <code
                            class="flex-1 text-xs bg-muted px-3 py-2 rounded font-mono break-all"
                        >
                            {createdWebhook?.url ?? "—"}
                        </code>
                        <Button
                ...
                {#if plainSecret}
                    <div
                        class="rounded-md bg-yellow-500/10 border border-yellow-500/30 p-3 space-y-2"
                    >
                        ...
                        <div class="flex items-center gap-2">
                            <code
                                class="flex-1 text-xs font-mono break-all bg-background px-2 py-1.5 rounded border"
                            >
                                {plainSecret}
                            </code>
                            <Button
                    ...
                    <ol class="space-y-1.5">
                        {#each guideSteps as s, i (i)}
                            <button
                                ...
                                tabindex="0"
                                onkeydown={(e) =>
                                    e.key === "Enter" && (guideStep = i)}
                            >
                                <span
                                    class="flex items-center justify-center size-5 rounded-full text-xs font-semibold shrink-0 mt-0.5
                                    {guideStep > i
                ...
                <div class="rounded-lg border bg-muted/30 p-3 flex gap-2.5">
                    ...
                    <p class="text-xs text-muted-foreground leading-relaxed">
                        After saving in {providerLabel}, a ping will be sent to
                        your webhook URL. Check
                        <span class="font-medium text-foreground"
                            >Delivery history</span
                        >
                        on the webhook row — a <Badge
                            variant="outline"
                            class="text-xs py-0 px-1">pong</Badge
