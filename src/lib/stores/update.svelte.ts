...
function createUpdateStore() {
  ...
  async function triggerUpdate() {
    ...
    try {
      ...
      // updating stays true until onUpdateDone/onUpdateFailed is called via WS
    } catch (e) {
      updating = false;
  ...
  const planeComponent = $derived(
    info?.components.find((c) => c.name === "plane") ?? null,
  );
  ...
  return {
    ...
    get releaseNotes() {
      return (
        info?.components.find((c) => c.update_available)?.changelog ?? null
      );
    },
    check,
    fetchNotes,
